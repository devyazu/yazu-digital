...
function normalizeCopy1Result(raw) {
  ...
  return { hooks: uniqueHooks.slice(0, 10), scriptOpeners: scriptOpeners.slice(0, 3) };
}

async function runCopy1(ai, prompt) {
  const response = await ai.models.generateContent({
    model: 'gemini-2.0-flash',
    contents: prompt,
    config: { temperature: 0.4, responseMimeType: 'application/json' },
  });
  return normalizeCopy1Result(parseJsonSafely(response?.text || ''));
}

async function handleSimpleGenerate(req, res) {
  const { prompt, systemInstruction } = req.body || {};
  if (!prompt || typeof prompt !== 'string') return send(res, 400, { error: 'Missing or invalid "prompt"' });
  try {
    ...
    return send(res, 200, { text: response.text ?? 'No response generated.' });
  } catch (err) {
    console.error('Gemini API Error:', err);
    return send(res, 500, { error: 'An error occurred while communicating with the AI. Please try again.' });
  }
}
...
async function handleToolRun(req, res) {
  ...
  const brandId = req.body?.brandId ? String(req.body.brandId).trim() : null;
  ...
  if (creditsTotal - creditsUsed < COPY1_COST) return send(res, 402, { error: 'Insufficient credits for this tool run' });
  ...
  if (!structured) structured = await runCopy1(ai, `${prompt}\n\nIMPORTANT: Strict JSON only. Do not include markdown.`);
  if (!structured) {
    await supabaseAdmin.from('tool_runs').insert({
      user_id: auth.userId, brand_id: brandId, tool_id: 'copy-1', status: 'failed',
      error_message: 'response_validation_failed', input_preview: input.slice(0, 500), cost_used: 0, latency_ms: Date.now() - startedAt,
    }).then(() => {}).catch(() => {});
    return send(res, 502, { error: 'Model response validation failed. Please retry.' });
  }

  const latencyMs = Date.now() - startedAt;
  const { error: creditErr } = await supabaseAdmin.from('profiles').update({ credits_used: creditsUsed + COPY1_COST }).eq('id', auth.userId);
  if (creditErr) return send(res, 500, { error: 'Could not update credits' });
  await supabaseAdmin.from('tool_runs').insert({
    user_id: auth.userId, brand_id: brandId, tool_id: 'copy-1', status: 'success',
    error_message: null, input_preview: input.slice(0, 500), cost_used: COPY1_COST, latency_ms: latencyMs,
  }).then(() => {}).catch(() => {});

  return send(res, 200, { toolId: 'copy-1', cost: COPY1_COST, output: structured, usage: { latencyMs } });
}
...
export default async function handler(req, res) {
  ...
  if (!GEMINI_API_KEY) return send(res, 503, { error: 'API Key is missing. Set GEMINI_API_KEY in Vercel Environment Variables.' });

  const action = String(req.body?.action || '').trim();
  if (action === 'tool-run') return handleToolRun(req, res);
  return handleSimpleGenerate(req, res);
}
